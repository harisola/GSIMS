...
@elseif($approval_data['adjustment_type']=="Exceptional Adjustment") 
    <tr class="odd gradeX">
        <td style="text-align:center;vertical-align:middle;">{{$count+1}}</td>
        <td style="vertical-align:middle;">
            <span class="staffImg"><img class="user-pic rounded tooltips" data-container="body" data-placement="top" data-original-title="18-052" src="assets/photos/hcm/150x150/staff/{{$approval_data['employee_id']}}.png"></span>
...
@else
    ...
    <tr class="odd gradeX">
        ...
        <td class="ExceptionalAdjEvent">
            ...
            <span class="MissedTap">
                ...
                <?php $approval_dates=explode('///',$approval_data['no_of_days']);
                 ...
                 ?>
                 @if($from_date)
                <span><strong class="light">From Date: </strong>{{$from_date}}</span><br />
                <span><strong class="light">To Date: </strong>{{$to_date}}</span><br />
                @endif
                @if($approval_dates[2])
                    <span><strong class="light">From Time: </strong>{{date('h:i:s A', strtotime($approval_dates[2]))}}</span><br />
                    <span><strong class="light">To Time: </strong>{{date('h:i:s A', strtotime($approval_dates[3]))}}</span><br />
                @endif
        ...
        @if($approval_data['approval_status']==0)
                <div class="div_{{$approval_data['effected_table_id']}}">
                    <span class="pointer" 
                    onclick="updateLeave({{$approval_data['effected_table_id']}},{{$approval_data['id']}})"
