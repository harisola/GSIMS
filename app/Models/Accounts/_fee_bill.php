...
class fee_bill extends Model
{
    ...
    public function feeInformationFilter($academic_session_id,$billing_cycle_number="",$grade_id,$section_name="",$gs_id="",$gf_id="",$gt_id=""){
              ...
              if(!empty($grade_id) && empty($section_id) && empty($gs_id) && empty($gf_id) && empty($gt_id)){
                    ...
                    ->whereIN("std_info.std_status_code",['S-CFS','S-CPT','F-LLV','F-NAD','S-WNT'])
    ...
    public function getLastBillByStudentId($student_id){
    	$details=fee_bill::where('student_id',$student_id)->select('id','total_payable','total_current_bill','roll_over_charges')->Orderby('id','desc')->first();
    	return $details;
    }
