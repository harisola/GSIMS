...
class Haris extends StaffReportController
{
    public function development(){
        ...
        /**** org - page ****/
        return view('master_layout.staff.staff_view2')->with(array('staff' => $staff, 'user' => $user, 'filter' => $staffFilter, 'staffCategory' => $staffCategory,'leaveType' => $leaveType));
    }
    ...
    ***********************************************************************/
    public function addAbsentia(Request $request){
        ...
        if($staff_absentia_in > 0 && $staff_absentia_out > 0){
           ...
           $absentia =  $staffInfo->get('atif_gs_events.daily_attendance_report',$where);

           

           if(count($absentia) != 0){
            $data = $staffInfo->generateReportByStaff($date,$staff_id);
            $this->generateAttendanceReport($data,$date,'update');
           }
        }
  ...
  public function addManual(Request $request){
    ...
    ***********************************************************************/
    ...
    public function getDailyReport(Request $request){
      ...
      $date = $request->input('date');
      $staff_id = $request->input('staff_id');
      $getReport = $staffInfo->getDailyReportData($date,$staff_id);
