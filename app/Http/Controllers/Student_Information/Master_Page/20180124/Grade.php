...
class Grade extends Controller
{
	public function Students(Request $request)
	{
		...
		return view('master_layout.student.student_view')->with(array('Students' => $Students, "Fence" =>$Fence, "Tchrs" => $Teachers, "_Badges" => $Badges, 'sectionID' => $Section,"userInfo"=>$userInfo, "School_level_Student"=>$School_level_Student, "Attendance_Score"=>$Attendance_Score ));
	}
...
public function Student_Stories(Request $request)
{
	...
	#return view('master_layout.student.student_stories')->with(['students'=>$result, 'stories'=>$stories, "Stories_limit"=>$Stories_limit, 'login_user'=>Sentinel::getUser()->id,"abridged_name_staff" => $abridged_name, "Current_user_pic"=>$Current_user_pic]);
...
public function Students_Stories( Request $request )
{
    ...
    $stories = $this->Get_Student_Stories($Student_id,0, 30);
    $i = 0;
    foreach ($stories as $u) {
		$pic = $this->get_Staff_Pic($u->actions, $u->gender);
	    $stories[$i]->photo500 = $pic['photo500'];
...
public function Student_Stories_Filter(Request $request){
    ...
    //var_dump($Search_Cat_Finance);

    $Where = '';
    if($Sort_By_Source != '' && $Sort_By_Source == 'system'){
		$Where .= ' ( actions = 0 OR actions = 298 ) AND ';		
	}else{
    ...
    if(isset($Search_Cat_Finance)){ $Where .= '(';}
    ...
    if($Where != '' && isset($Search_Cat_Finance)){
    ...
    $stories = $this->getFilteredStories($stories_start, $stories_limit, $Where);
...
public function getFilteredStories($lowerLimit, $upperLimit, $Where)
{
	$query = "select
	thisData.*
	from
		(select 
			...
			'gs_logo' as actions, 'M' as gender, 1 as user_id
...
public function getStories( $lowerLimit, $upperLimit){
	...
	from
		(select 
			...
			'SMS' as tag, '' as communication_tag, 'gs_logo' as actions, 'M' as gender, 0 as user_id
...
public function getStories2( $lowerLimit, $upperLimit){
	...
	from
		(select 
			cl.gs_id, cl.abridged_name, 'Generations School' as staff_name,
			sms.message as comments, DATE_FORMAT(sms.created, '%a, %d %b %Y - %h:%i %p') as date_time, sms.created as thisDateTime,
			'SMS' as tag, '' as communication_tag, 'gs_logo' as actions, 'M' as gender, 1 as user_id
...
select
	cl.gs_id as gs_id, 
			...
			#FROM_UNIXTIME( UNIX_TIMESTAMP ( DATE_FORMAT(concat( att.date, ' ', att.time),'%a, %d %b %Y - %h:%i %p' ) ) )as date_time,
...
group by vt.date ";
	...
	public function add_student_story(Request $request){
		...
		$html = '<div class="portlet box blue-hoki">
		 ...
		 <select data-attribute="profile" multiple="multiple" id="Dropdown_Comment_Category" class="ddlFilterTableRow layout-option form-control input-sm">
	...
	<div class="btn-group">
	<button type="button" class="multiselect dropdown-toggle btn btn-default" data-toggle="dropdown" title="Comment Category" aria-expanded="true">
	<span class="multiselect-selected-text">Select Comment Category </span>
	<b class="caret"></b></button>
	<ul class="multiselect-container dropdown-menu">
	<li><a tabindex="0"><label class="checkbox" for="Comment_Cat[]"><input type="checkbox" name="Comment_Cat[]" value="Finance" id="Finance"> Finance</label></a></li>
	<li><a tabindex="1"><label class="checkbox" for="Academic"><input type="checkbox" name="Comment_Cat[]" value="Academic" id="Academic"> Academic</label></a></li>
	<li><a tabindex="2"><label class="checkbox" for="E-Tab"><input type="checkbox" name="Comment_Cat[]" value="E-Tab" id="E-Tab"> E-Tab</label></a></li>
	<li><a tabindex="3"><label class="checkbox" for="SIS"><input type="checkbox" name="Comment_Cat[]" value="SIS" for="SIS">SIS</label></a></li>
	<li><a tabindex="4"><label class="checkbox" for="Family"><input type="checkbox" name="Comment_Cat[]" value="Family" for="Family">Family</label></a></li>
	<li><a tabindex="5"><label class="checkbox" for="SMS"><input type="checkbox" name="Comment_Cat[]" value="SMS" for="SMS">SMS</label></a></li>
	 ...
	 <span class="multiselect-native-select">
		 <select data-attribute="profile" multiple="multiple" id="Dropdown_Comm_Category" class="ddlFilterTableRow layout-option form-control input-sm">
	...
	<div class="btn-group">
	<button type="button" class="multiselect dropdown-toggle btn btn-default" data-toggle="dropdown" title="Communication Category" aria-expanded="true">
	<span class="multiselect-selected-text">Select Communication Category </span>
	<b class="caret"></b></button>
	<ul class="multiselect-container dropdown-menu">
	<li><a tabindex="0"><label class="checkbox" for="Cmm_Cat[]"><input type="checkbox" name="Cmm_Cat[]" value="Call"> Call</label></a></li>
	<li><a tabindex="1"><label class="checkbox" for="Cmm_Cat[]"><input type="checkbox" name="Cmm_Cat[]" value="SMS"> SMS</label></a></li>
	<li><a tabindex="2"><label class="checkbox" for="Cmm_Cat[]"><input type="checkbox" name="Cmm_Cat[]" value="WhatsApp"> WhatsApp</label></a></li>
	<li><a tabindex="3"><label class="checkbox" for="Cmm_Cat[]"><input type="checkbox" name="Cmm_Cat[]" value="Walkin">Walkin</label></a></li>
                                        ...
                                        <!--select data-attribute="profile" multiple="multiple" id="studentComCateo" class="ddlFilterTableRow layout-option form-control input-sm">
                                           <option value="Call">Call</option>
                                           <option value="SMS">SMS</option>
                                           <option value="WhatsApp">WhatsApp</option>
                                           <option value="Walkin">Walkin</option>
