...
public function Student_Stories(){
	$query = "select
	cl.gs_id, cl.abridged_name, cl.call_name, cl.grade_name, cl.section_name, cl.gr_no as Photo_id,
	...
	return view('master_layout.student.student_stories')->with(['students'=>$result, 'stories'=>$stories, 'login_user'=>Sentinel::getUser()->id]);
...
public function Student_Stories_Limit(Request $request){
    ...
    $stories = $this->getStories($stories_start, $stories_limit);
    ...
    foreach ($stories as $u) {
	    ...
	    if (Sentinel::getUser()->id != $u->user_id){
	    	...
	    	$html .= '<li class="out" data-filters="User Generated" data-category="Accounts">
                        ...
                        <div class="message"><span class="arrow"> </span>
                            <div class="CommentInfo">
                                <a href="javascript:;" class="name"> <strong>Right Individual</strong> </a>
                                <span class="studentInfoCom"><span class="grayish">Student Name:</span>'.$u->abridged_name.'</span>
                                <span class="studentInfoCom"><span class="grayish">GS-ID:</span>'.$u->gs_id.'</span>
...
public function Student_Stories_Filter(Request $request){
    ...
    $Where = '';
    if($Sort_By_Source != '' && $Sort_By_Source == 'system'){		$Where .= ' (actions = 0 OR actions = 298) AND ';		}else{
    	$Where .= ' (actions != 0 AND actions != 298) AND ';
    }
    if($Sort_By_User != ''){		$Where .= ' (actions = ' . $Sort_By_User . ') AND ';	}
...
public function getFilteredStories($lowerLimit, $upperLimit, $Where){
	...
	from
		(select 
			cl.gs_id, cl.abridged_name, '' as staff_name,
			sms.message as comments, DATE_FORMAT(sms.created, '%a, %d %b %Y - %h:%i %p') as date_time, sms.created as thisDateTime,
			'SMS' as tag, '' as communication_tag, '298' as actions, 'M' as gender, 1 as user_id
	...
	public function add_student_story(Request $request){
		...
		$html = '<div class="portlet box blue-hoki">
                    <div class="portlet-title">
                        ...
                        <input type="hidden" id="date_time" value="'.date('Y-m-d H:i A').'">
                    </div><!-- portlet-title -->
                    <div class="portlet-body">
                        <div class="row">
                        	<div class="col-md-12">
