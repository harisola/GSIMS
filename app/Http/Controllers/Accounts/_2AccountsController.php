...
class AccountsController extends Controller
{
  ...
  // return response()->json(array('success' => true, 'Gname'=>$Gname,'Snames'=>$Snames,'st'=>$st ));
    ...
    public function StudentApplicableForTaxes($list,$total_current_bill){
            $fee_bill = new fee_bill;
            $class_list = new class_list;
            $session = new academic_session;
            $arriers_adjustment= new arriers_adjustment;
            ...
            $remitance_taxes=$arriers_adjustment->getAllRemitanceadjustements($list['student_id'])['adjustment_amount']; 
            ...
            $total_amount=$total_current_bill+$total_adjustments+$admission_fee;
   ...
   public function fetchFeeBill($gs_id){
            ...
            $pdf= new FPDI;
            $fpdf= new Fpdf;
            $pdf->setSourceFile(base_path().'/public/pdf/fee_bill.pdf');
	    	// $gs_id=$request->get('gs_id');
    ...
    //         $pdf->MultiCell( 200, 40,$text, 1);
           ...
           // 8 1s
            ...
            if($gt_id != null){
                $this->createTable($pdf,60,"GT-ID".$gt_id,6,'B',4);
            }
            ...
            // $this->createTable($pdf,135,$feedetails['additional_charges'],67,'',5);//additional charges
                ...
                if($oc_adv_tax!=0){
                    ...
                    $this->createTable($pdf,148,'-',67,'B',5);
                }

                $this->createTable($pdf,154,'Arrears / Adjustments',10,'B',5);
                $this->createTable($pdf,154,$feedetails['adjustment']+$feedetails['roll_over_charges'],67,'B',5);
                $this->createTable($pdf,161,$feedetails['total_payable'],67,'B',5);
                if($feedetails['arrears_suspended']){
   ...
   public function split_on($string, $num) {
    ...
    public function calculate_arrier_adjustments($student_id){
        ...
        return $total_payable=$fee_details['total_payable'];//get last bill total payable amount
